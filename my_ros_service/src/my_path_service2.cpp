...
const double g_move_speed = .5; // set forward speed to this value, e.g. 1m/s
const double g_spin_speed = .5; // set yaw rate to this value, e.g. 1 rad/s
const double g_sample_dt = .001;
...
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 double xNeed = goal_pose.position.x - current_pose.position.x;
 double yNeed = goal_pose.position.y - current_pose.position.y;
 double zNeed = goal_pose.orientation.z; 

 heading = zNeed;
 dist =  sqrt(pow(xNeed,2) + pow(yNeed,2));
}
...
//        yaw_desired = convertPlanarQuat2Phi(pose_desired.orientation); //from i'th desired pose
        ...
        yaw_current = g_current_pose.orientation.z; //our current yaw--should use a sensor convertPlanarQuat2Phi(
	...
	g_current_pose.position = pose_desired.position; //hopefully
     }
